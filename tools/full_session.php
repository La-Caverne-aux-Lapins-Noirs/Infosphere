...
class FullSession
{
	...
	transfert(
	    ["id", "begin_date", "end_date", "is_template", "id_template",
	     "template_link", "id_activity", "maximum_subscription", "deleted"
	    ], $this, $session
	);
	...
	$this->room = db_select_all("
           room.*,
           room.{$Language}_name as name
           FROM session_room
           LEFT JOIN room ON session_room.id_room = room.id
           WHERE session_room.id_session = {$session["id"]}
	   ", "name");
	foreach ($this->room as $rom)
	{
	    if ($rom["capacity"] == -1)
	    {
		$this->room_space = -1;
	...
	$this->current_occupation = $this->parent->team_size * count($this->team);
