...
function collect_dashboard_projects($now, $since = 60 * 60 * 24, $past_projs = false)
{
    global $Language;
    global $User;
    ...
    if (!$past_projs)
	$datelimit =
	    "activity.registration_date <= '".db_form_date($now - $since)."'".
            "AND activity.pickup_date >= '".db_form_date($now)."'"
    ;
    else
	$datelimit =
	    "activity.pickup_date >= '".db_form_date($now - $since)."'".
            "AND activity.pickup_date <= '".db_form_date($now)."'"
    ;
	...
	$sess["medal"] = count($s->medal);
	$sess["medal_got"] = 0;
	foreach ($s->medal as $med)
	    $sess["medal_got"] += $med["result"] > 0;
