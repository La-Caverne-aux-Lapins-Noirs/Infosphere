...
for ($i = 0; $i < 6; ++$i)
{
	...
	break ;
    imageline($img,
	      ($i + 0.5) * $w / 6, $h * 0.7 - $h * 0.55 * $percents[$i],
	      ($i + 1.5) * $w / 6, $h * 0.7 - $h * 0.55 * $percents[$i],
	      $white);
    imagettftext($img, 10, 0,
		 ($i + 0.5) * $w / 6 + 5, $h * 0.7 - $h * 0.55 * $percents[$i] - 5,
	...
	$img,
	($i + 1 - 0.2) * $w / 6, $h * 0.7 - $h * 0.55 * $score[$i],
	($i + 1 + 0.2) * $w / 6, $h * 0.7,
	$ccol
    );
...
imagefilledrectangle(
    $img,
    0.5 * $w / 6, $h * 0.85,
    0.5 * $w / 6 + $w * ($final_grade) / 6, $h * 0.95,
