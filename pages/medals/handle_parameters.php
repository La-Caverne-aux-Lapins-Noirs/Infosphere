...
if ($unique == false)
{
    $fetch = @fetch_data("medal", $fetch, ["name", "description"], "codename", $sort, true, false, $where, ["tags"], $pagelen, $page);
    $meds = &$fetch->value;
}
else
{
...
foreach ($meds as $i => &$m)
{
	...
	medal.codename, medal.id, medal_medal.id as id_medal_medal,
        medal_medal.nbr, medal.icon
        FROM medal_medal
        LEFT JOIN medal
        ON medal_medal.id_implied_medal = medal.id
        WHERE medal.deleted = 0 AND medal_medal.id_medal = ".$m["id"]."
	");
}
