...
>
    ...
    $display_subject = $activity->subject != "" && ($activity->is_teacher || $activity->registered && $not_too_soon && $not_too_late);
    ?>

    <h4><?=$Dictionnary["ActivitySubject"]; ?></h4>
    <?php $pdf = substr($activity->subject, -4, 4) == ".pdf"; ?>

    <?php if ($display_subject) { ?>
	<div style="float: right; height: 25px;">
	    <?php if ($pdf) { ?>
		<a href="<?=$activity->subject; ?>">
		    <?=$Dictionnary["Download"]; ?>
		</a>
	    <?php } ?>
	</div>
	<iframe
	    src="<?=$activity->subject; ?><?=$pdf ? '#toolbar=0&navpanes=0&scrollbar=0' : ''; ?>"
